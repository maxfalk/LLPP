...
void Ped::Crowd::init_cuda(){
  ...
  calc_max_cuda_threads(); //Calc threads used per block  
  ...
  //  Copy
  cudaMemcpy(d_DesiredX, DesiredX, size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_DesiredY, DesiredY, size, cudaMemcpyHostToDevice);
...
void Ped::Crowd::calc_max_cuda_threads(){

  cudaThreads = MAXCUDATHREADS;
  while(NumberOfAgents % cudaThreads != 0){
    cudaThreads--;
  }
...
void Ped::Crowd::go_cuda(){
  ...
  int size = NumberOfAgents * sizeof(float);
  int threadsPerBlock = cudaThreads; 
  int blocksPerGrid = NumberOfAgents / threadsPerBlock;

  kernel_go(blocksPerGrid, threadsPerBlock,
	    d_DesiredX, d_MoveForceX,
	    d_DesiredY, d_MoveForceY,
	    NumberOfAgents);
 
  cudaMemcpy(DesiredX, d_DesiredX, size, cudaMemcpyDeviceToHost);
  cudaMemcpy(DesiredY, d_DesiredY, size, cudaMemcpyDeviceToHost);
}
void Ped::Crowd::where_to_go_cuda(){ 
  ...
  cudaMemcpy(d_DesiredX, AgentsX, size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_DesiredY, AgentsY, size, cudaMemcpyHostToDevice);
